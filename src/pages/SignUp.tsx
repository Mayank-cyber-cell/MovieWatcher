...
export const SignUp: React.FC<SignUpProps> = ({ isDarkMode, onSignUp, loading = false }) => {
    ...
    <div className={`min-h-screen transition-colors duration-300 flex items-center justify-center px-4 ${
      ...
      <div className={`w-full max-w-md transition-all duration-300 ${
        ...
        <p className={`text-center mb-6 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
          Join us to discover and save your favorite movies
        </p>
        ...
        {success && (
          <div className={`mb-4 p-4 rounded-lg flex gap-3 ${
            ...
            <p className="text-green-600">Account created! Redirecting to sign in...</p>
          </div>
        )}
        ...
        {!success && (
          <p className={`mt-6 text-center ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            ...
            <Link to="/signin" className="font-semibold text-blue-400 hover:text-blue-300 transition-colors duration-300">
              Sign in
            </Link>
          </p>
        )}
